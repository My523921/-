...
def kick_async_loop(*args) -> bool:
    ...
    if not len(all_tasks):
        ...
        log.debug('all %i tasks are done, fetching results and stopping after this kick.',  # noqa
                  len(all_tasks))
        stop_after_this_kick = True
...
class AsyncModalOperatorMixin:
    ...
    def modal(self, context, event):
        ...
        if self._state != 'EXCEPTION' and task and task.done() and not task.cancelled():  # noqa
            ex = task.exception()
            if ex is not None:
    ...
    def _new_async_task(self, async_task: typing.Coroutine, future: asyncio.Future = None):  # noqa
        """Stops the currently running async task, and starts another one."""

        self.log.debug(
            'Setting up a new task %r, so any existing task must be stopped', async_task)  # noqa
        self._stop_async_task()
